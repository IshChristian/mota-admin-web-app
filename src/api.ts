...
export const adminApi={stats:()=>api.get('/admin/stats'),users:()=>api.get('/admin/users'),drivers:()=>api.get('/admin/drivers'),registrations:()=>api.get('/admin/registrations'),transactions:()=>api.get('/admin/paypack/transactions'),roles:()=>api.get('/roles'),audits:()=>api.get('/audit-logs'),createRole:(data:{name:string;description:string;permissions:string[]})=>api.post('/roles',data),updateRole:(id:string,permissions:string[])=>api.put(`/roles/${id}/permissions`,{permissions}),deleteRole:(id:string)=>api.delete(`/roles/${id}`)};

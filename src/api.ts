...
export type UserInput = Partial<UserRecord> & {
  firstName: string;
  lastName: string;
  phone: string;
  password?: string;
...
export const adminApi = {
  ...
  createUser: (data: UserInput) => api.post("/admin/users", data),
  ...
  assignUserRole: (id: string, role: string, roleId?: string) =>
    api.patch(`/admin/users/${id}/role`, { role, roleId }),
  deleteUser: (id: string) => api.delete(`/admin/users/${id}`),
  setUserActive: (id: string, isActive: boolean) =>
    api.put(`/admin/users/${id}/status`, { isActive }),
  verifyUser: (id: string) => api.put(`/admin/users/${id}/verify`),
  setKyc: (id: string, kycLevel: "basic" | "full") =>
    api.put(`/admin/users/${id}/kyc`, { kycLevel }),
  ...
  reviewRegistration: (id: string, status: string, remarks: string) =>
  ...
  updateConfig: (key: string, value: unknown, description?: string) =>
    api.post("/admin/configs", { key, value, description }),
  rides: (params?: Record<string, unknown>) =>
    api.get("/admin/rides", { params }),
  cancelRide: (id: string, reason: string) =>
    api.patch(`/admin/rides/${id}/cancel`, { reason }),
  loans: () => api.get("/loans/admin/all", { params: { limit: 100 } }),
  decideLoan: (loanId: string, action: "approve" | "reject") =>
    api.post("/loans/admin/approve", { loanId, action }),
  fines: () => api.get("/fine-requests/all", { params: { limit: 100 } }),
  approveFine: (id: string, notes?: string) =>
    api.put(`/fine-requests/${id}/approve`, { notes }),
  ...
  deleteSupportCase: (id: string) => api.delete(`/admin/support-cases/${id}`),
  supportOperations: () => api.get("/admin/support-operations"),
  logSupportContact: (id: string, data: Record<string, unknown>) =>
    api.post(`/admin/support-cases/${id}/contact`, data),
  notifySupportPassenger: (id: string) =>
    api.post(`/admin/support-cases/${id}/notify-passenger`),
  assignSupportRide: (
    rideId: string,
    driverId: string,
    supportCaseId?: string,
  ) =>
    api.post(`/admin/support-rides/${rideId}/assign-driver`, {
      driverId,
      supportCaseId,
    }),
  createSupportRide: (data: Record<string, unknown>) =>
